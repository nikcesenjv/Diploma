...
from .parlamint_attendee_management import is_attendee, create_parlamint_attendee
...
from src.objects.parlamint_objects import ParlamintAttendee, ParlamintNote, ParlamintSpeaker, ParlamintUtterance, \
    ParlamintHead, ParlamintSpeakerList

def create_parlamint_note(note_type: str) -> ParlamintNote:
    return ParlamintNote(note_type)

def create_parlamint_speaker(speaker: str, parlamint_utterance: ParlamintUtterance) -> ParlamintSpeaker:
    parlamint_note = create_parlamint_note("speaker")
    parlamint_note.add_paragraph(speaker)
    return ParlamintSpeaker(parlamint_note, parlamint_utterance)

def create_parlamint_utterance(attendee: ParlamintAttendee) -> ParlamintUtterance:
    parlamint_utterance = ParlamintUtterance(attendee)
    return parlamint_utterance

def create_parlamint_head(head_type: str) -> ParlamintHead:
    return ParlamintHead(head_type)

def create_parlamint_speaker_list() -> ParlamintSpeakerList:
    return ParlamintSpeakerList()

def create_parlamint_property_element(property_element: dict[str, str | list]) \
        -> ParlamintHead | ParlamintNote | ParlamintSpeakerList:

    match property_element["tag"]:
        case "head":
            return create_parlamint_head(property_element["type"])
        case "note":
            return create_parlamint_note(property_element["type"])
        case "speakers":
            return ParlamintSpeakerList()
...
def parse_parlamint_properties(properties: dict[str, str | list], paragraphs: list[Paragraph],
    ...
    for i, property_element in enumerate(properties):
        if "skip" in property_element and property_element["skip"]:
        ...
        try:
            ...
            if "role" in property_element:
                parlamint_attendee_list = parse_parlamint_attendees(parlamint_attendee_list, attendees,
    ...
    for i, paragraph in enumerate(paragraphs):
        try:
            ...
            if is_close_match_list(paragraph, properties["similar words"]):
                element_paragraphs.append(paragraph)
            elif closest_substring(paragraph, " ".join(properties["similar words"])) is not None:
                element_paragraphs.append(paragraph)

            return element_paragraphs, []

def parse_parlamint_attendees(parlamint_document_attendees: list[ParlamintAttendee],
                              all_attendees: list[ParlamintAttendee], paragraphs: list[str]) \
        -> tuple[list[ParlamintAttendee], list[ParlamintAttendee]]:

    # attendee_names = extract_attendees_from_string(" ".join(paragraphs))
    attendee_names = extract_attendees_from_string(paragraphs)

    for attendee_name in attendee_names:
        if is_attendee(attendee_name, all_attendees) or is_attendee(attendee_name, all_attendees):
            continue
...
def create_parlamint_note(note_type: str, string: str) -> ParlamintNote:
...
def parse_parlamint_speakers(paragraphs: list[Paragraph], attendees: list[ParlamintAttendee]) -> list[ParlamintSpeaker]:
    ...
    for i, paragraph in enumerate([paragraph for paragraph in paragraphs[:-1] if paragraph != ""]):
        ...
        x = closest_substring(paragraph, "čita", 90)
        if x is not None:
